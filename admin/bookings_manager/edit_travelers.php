...
$travelers_ID = isset($_REQUEST['id']) ? $_REQUEST['id']: 0;
$sql = "select t.*, b.bookings_summary from ".$db_suffix."travelers t LEFT JOIN ".$db_suffix."bookings b on b.bookings_ID=t.bookings_ID where travelers_ID = $travelers_ID limit 1";				
$query = mysqli_query($db, $sql);

if(mysqli_num_rows($query) > 0)
{
	$content     = mysqli_fetch_object($query);
	$travelers_first_name       = $content->travelers_first_name;
...
<h3 class="page-travelers_first_name">
    Reisender zum Buchung aktualisieren
</h3>
<div class="page-bar">
                ...
                <div class="form-body">
                    ...
                    <form action="<?php echo str_replace('&s_factor=1', '', $_SERVER['REQUEST_URI']);?>" class="form-horizontal" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-2">
                                <input type="text" class="form-control" disabled value="<?php echo $travelers_update_time;?>" />
                            </div>
                        </div>
