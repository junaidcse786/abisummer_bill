...
{	
	extract($_POST);
	//echo "SELECT rp_ID from ".$db_suffix."rooms_price where hotels_ID = $hotels_ID AND rooms_ID = $rooms_ID AND rp_price_range=''";
	
    if(empty($date_from) && empty($date_to)):
    
        if(mysqli_num_rows(mysqli_query($db, "SELECT rp_ID from ".$db_suffix."rooms_price where hotels_ID = $hotels_ID AND rooms_ID = $rooms_ID AND (rp_price_date_from='0000-00-00' AND rp_price_date_to='0000-00-00')"))>0)
        {
	...
	if(!empty($date_from) && mysqli_num_rows(mysqli_query($db, "SELECT rp_ID from ".$db_suffix."rooms_price where hotels_ID = $hotels_ID AND rooms_ID = $rooms_ID AND (rp_price_date_from='$date_from' OR rp_price_date_to='$date_from')"))>0):		
	...
	if(!empty($date_to) && mysqli_num_rows(mysqli_query($db, "SELECT rp_ID from ".$db_suffix."rooms_price where hotels_ID = $hotels_ID AND rooms_ID = $rooms_ID AND (rp_price_date_from='$date_to' OR rp_price_date_to='$date_to')"))>0):		
...
<!-----PAGE LEVEL CSS END--->
                                        ...
                                        <h3 class="page-rp_price_date_from">
                                                Zimmerpreis für diesen Hotel einfügen
                                        </h3>
                                        <div class="page-bar">         
                                        <ul class="page-breadcrumb">
                                                ...
                                                <li>
                                                        <a href="<?php echo SITE_URL_ADMIN.'?mKey='.$mKey.'&pKey=rooms_price&id='.$hotels_ID; ?>">Zimmerpreis Liste für diesen Hotel</a>
                                                </li>
                                        </ul>
                          ...
                          <div class="alert alert-<?php echo $alert_type; ?> <?php echo $alert_box_show; ?>">
                            ...
                            <?php echo $alert_message; ?>
                               ...
                               <div class="form-group <?php echo $messages["rooms_ID"]["status"] ?>">
                                 <label for="parent" class="control-label col-md-3">Zimmertyp <span class="required">*</span></label>
                                 <div class="col-md-4">
